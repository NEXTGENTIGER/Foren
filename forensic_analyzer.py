...
class ForensicAnalyzer:
    ...
    def extract_metadata(self) -> None:
        ...
        try:
            result = self._safe_subprocess_run(
                [self._get_tool_command('exiftool'), "-json", str(self.target_file)],
                capture_output=True,
                text=True
            )
    ...
    def analyze_sleuthkit(self) -> None:
        ...
        try:
            ...
            fls = self._safe_subprocess_run(
                [self._get_tool_command('sleuthkit'), str(self.target_file)],
                capture_output=True,
                text=True
            )
            if fls.returncode == 0:
    ...
    def run_bulk_extractor(self) -> None:
        ...
        try:
            ...
            result = self._safe_subprocess_run(
                [self._get_tool_command('bulk_extractor'), "-o", output_dir, str(self.target_file)],
                capture_output=True,
                text=True
            )
            if result.returncode == 0:
...
def main():
    ...
    try:
        with ForensicAnalyzer(args.file, args.api_endpoint, args.output_dir, logger) as analyzer:
            report = analyzer.run_analysis(args.yara_rules)
            
            # Envoi du rapport à l'API si demandé
            if not args.no_upload:
                if analyzer.send_report():
                    logger.info("Rapport envoyé avec succès à l'API")
                else:
                    logger.error("Échec de l'envoi du rapport à l'API")
    except Exception as e:
